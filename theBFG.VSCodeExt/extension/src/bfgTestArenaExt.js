...
let currentUrl = 'http://localhost:888';
function activate(context) {
    context.subscriptions.push(vscode.commands.registerCommand('bfg.connect', async () => {
        if (webviewPanel) {
            webviewPanel.dispose();
        }
        // Create webview panel
        webviewPanel = vscode.window.createWebviewPanel('theBFGTestArena', 'theBFG Test Arena', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        webviewPanel.webview.onDidReceiveMessage(message => handleMessage(message), undefined, context.subscriptions);
    }));
}
