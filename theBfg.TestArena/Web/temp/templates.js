angular.module('portal').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('systemstatus/partial/testArena/testArena.html',
    "<div class=col-md-12 ng-controller=testArenaCtrl><form name=remote><div class=grouped><div><span ng-if=!showTestRunDetailed ng-repeat=\"entry in testRuns\"><span ng-if=\"!entry.completedAt && entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-space-shuttle\" ng-attr-title={{entry.dll}}></span> <span ng-if=\"!entry.completedAt && !entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-question-circle\" ng-attr-title={{entry.dll}}></span> <span ng-if=\"entry.completedAt && entry.results[0].failed !== 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-thumbs-o-down error\" ng-attr-title=\"{{entry.results[0].failed}} / {{(entry.results[0].passed + entry.results[0].failed)}} in {{entry.results[0].duration}}\"></span> <span ng-if=\"entry.completedAt && entry.results[0].failed === 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-trophy infor\" ng-attr-title=\"{{entry.results[0].failed}} / {{(entry.results[0].passed + entry.results[0].failed)}} in {{entry.results[0].duration}}\"></span></span><table ng-if=showTestRunDetailed class=\"table table-hover table-striped table-condensed h6\"><tbody><tr ng-repeat=\"entry in testRuns\" ng-init=\"showLogs = false\"><td width=5% ng-class=\"{none: !entry.completedAt && entry.total, error: entry.results.length > 0 && entry.results[0].failed !== 0, infor: entry.results.length > 0 && entry.results[0].failed === 0}\"><span ng-if=\"!entry.completedAt && entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-space-shuttle\" ng-attr-title=\"In Progress...\"></span> <span ng-if=\"!entry.completedAt && !entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-question-circle\" ng-attr-title=\"Not run yet...\"></span> <span ng-if=\"entry.completedAt && entry.results[0].failed !== 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-thumbs-o-down error\" ng-attr-title=\"{{entry.results.length}} test outcomes\"></span> <span ng-if=\"entry.completedAt && entry.results[0].failed === 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-trophy infor\" ng-attr-title=\"{{entry.results.length}} test outcomes\"></span></td><td width=5%><span title=Reload ng-click=\"publish('', 'StartUnitTest '+ entry.dll)\" class=\"cmd fa fa-rocket\"></span> <span class=popuptrigger ng-click=\"showLogs = !showLogs\">{{entry.dll}}<div class=popup><span class=popuptext ng-class=\"{show: showLogs}\"><span ng-repeat=\"a in entry.assets\"><a ng-href=\"{{'/tenantLogs/' + a}}\" target=_tenantlogs><span class=\"cmd fa fa-stack-overflow\" ng-attr-title={{a}}></span></a></span></span></div></span></td><td width=5% ng-if=\"entry.completedAt && entry.results[0].failed !== 0\">{{entry.results[0].failed}} / {{(entry.results[0].failed + entry.results[0].passed)}}</td><td width=5% ng-if=\"!entry.completedAt && entry.total !== 0\">{{entry.total}}</td><td width=5% ng-if=\"entry.completedAt && entry.results[0].failed === 0\">{{(entry.results[0].passed)}}</td><td width=5% ng-attr-title={{entry.results[0].duration}}>{{entry.results[0].duration}}</td><td width=80%><bar-chart class=cmd height=30 data=entry.results></bar-chart></td></tr></tbody></table></div><div class=\"pieChart gfirst\"><pie-chart class=cmd ng-click=\"cmdShowTopic('slow')\" data=testOutcomes></pie-chart></div><div class=\"barchart gsecond\"><bar-chart class=cmd height=250 ng-click=\"cmdShowTopic('log')\" data=testSummary></bar-chart></div></div><div><worker-status data=workerInfo></worker-status></div><div><span width=40><span title=Reload ng-click=cmdReload() ng-class=\"{error: !testArenaInfo.isConnected, infor: testArenaInfo.isConnected}\" class=\"cmd fa fa-rocket fa-2x\"></span> <i title=Errors ng-click=\"cmdShowTopic('Failed')\" class=cmd style=\"font-size:larger; padding-right:5px; padding-left: 5px\" ng-class=\"{error : testGlance.errors != '0', infor : testGlance.errors == '0'}\">{{testGlance.errors}}</i></span> / <span class=cmd ng-click=\"cmdShowTopic('all')\">{{testGlance.total}}</span> <span><span ng-click=\"cmdShowTopic('slow')\" class=\"cmd fa fa-truck fa-2x\" title=\"Slow: Duration > 100ms\"></span>{{testGlance.slow}}</span> <span><span ng-click=\"cmdShowTopic('flakey')\" class=\"cmd fa fa-rmb fa-2x\" title=Flakey></span> {{testGlance.flakey}}</span> <span><span ng-click=\"cmdShowTopic('log')\" class=\"cmd fa fa-clock-o fa-2x\"></span> {{msSince(testGlance.startedAt)}}</span> <span><span style=float:right ng-click=cmdClear() title=\"CLEAR ALL DATA!!!\" class=\"cmd fa fa-scissors fa-2x\"></span></span> <span><span class=\"cmd fa fa-search-plus fa-2x\"></span><input type=text ng-model=searchTerm width=\"50\"></span> <span><span ng-click=\"sendCmdDisabled = !sendCmdDisabled\" class=\"cmd fa fa-share-square-o fa-2x\" title=\"Send Custom Commands\"></span></span> <a href=/tenantLogs target=_tenantlogs><span class=\"cmd fa fa-stack-overflow fa-2x\"></span></a></div><div><span ng-if=!sendCmdDisabled style=color:cadetblue class=\"fa fa-share-square-o fa-2x\"></span><div ng-if=!sendCmdDisabled class=input-group style=\"padding-left:20px;padding-right:20px;padding-top: 5px;;padding-bottom: 5px\"><span class=input-group-btn style=\"width: 200px\"><input type=text class=form-control width=80 ng-init=\"cmd = 'Reload'\" ng-model=cmd> <input type=text class=form-control width=80 ng-init=\"destination = 'TestArena\\\\bfgWorker'\" ng-model=destination> <button class=\"cmd btn btn-default\" type=button ng-click=\"publish(destination, cmd)\">Send</button></span></div></div><div><span style=color:cadetblue ng-if=\"currentTopic != ''\" class=\"fa fa-search fa-2x\"></span><table ng-if=\"currentTopic != ''\" class=\"table table-hover table-striped table-condensed h6\"><thead><th>Outcome</th><th>Test</th><th>Worker</th><th>Duration</th><th>Log</th></thead><tbody><tr ng-class=\"{error: entry.result === 'Failed', infor: entry.result === 'Passed'}\" ng-init=\"showLogs = false\" ng-repeat=\"entry in currentTopic\"><td>{{entry.result}}</td><td><span class=popuptrigger ng-click=\"showLogs = !showLogs\">{{entry.testName}}</span><div class=popup><span class=popuptext ng-class=\"{show: showLogs}\"><span ng-repeat=\"a in entry.assets\"><a ng-href=\"{{'/tenantLogs/' + a}}\" target=_tenantlogs><span class=\"cmd fa fa-stack-overflow\" ng-attr-title={{a}}></span></a></span></span></div></td><td>{{entry.worker}}</td><td>{{entry.duration}}</td><td style=\"white-space: pre\" ng-if=entry.logMessage><pre class=stacktrace>{{entry.logMessage}}</pre></td><td ng-if=!entry.logMessage><i></i></td></tr></tbody></table><table ng-if=!logDisabled class=\"table table-hover table-striped table-condensed h6\"><thead><th>Outcome</th><th>Test</th><th>Worker</th><th>Duration</th><th>Log</th></thead><tbody><tr ng-class=\"{error: entry.result === 'Failed', infor: entry.result === 'Passed'}\" ng-repeat=\"entry in log | filter:searchTerm\"><td>{{entry.result}}</td><td><span class=popuptrigger ng-click=\"showLogs = !showLogs\">{{entry.testName}}</span><div class=popup><span class=popuptext ng-class=\"{show: showLogs}\"><span ng-repeat=\"a in entry.assets\"><a ng-href=\"{{'/tenantLogs/' + a}}\" target=_tenantlogs><span class=\"cmd fa fa-stack-overflow\" ng-attr-title={{a}}></span></a></span></span></div></td><td>{{entry.worker}}</td><td>{{entry.duration}}</td><td style=\"white-space: pre\" ng-if=\"entry.logMessage || entry.logMessage.length > 1\"><pre class=stacktrace>{{entry.logMessage}}</pre></td><td ng-if=\"!entry.logMessage || entry.logMessage < 2\"></td></tr></tbody></table></div></form></div>"
  );
