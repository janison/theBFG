angular.module('portal').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('systemstatus/partial/testArena/testArena.html',
    "<div class=col-md-12 ng-controller=testArenaCtrl><form name=remote><div class=grouped><div><span ng-if=!showTestRunDetailed ng-repeat=\"entry in testRuns\" style=\"position: fixed\"><span ng-if=\"!entry.completedAt && entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-space-shuttle\" ng-attr-title={{entry.dll}}></span> <span ng-if=\"!entry.completedAt && !entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-question-circle\" ng-attr-title={{entry.dll}}></span> <span ng-if=\"entry.completedAt && entry.results[0].failed !== 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-thumbs-o-down error\" ng-attr-title=\"{{entry.results[0].failed}} / {{(entry.results[0].passed + entry.results[0].failed)}} in {{entry.results[0].duration}}\"></span> <span ng-if=\"entry.completedAt && entry.results[0].failed === 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-trophy infor\" ng-attr-title=\"{{entry.results[0].failed}} / {{(entry.results[0].passed + entry.results[0].failed)}} in {{entry.results[0].duration}}\"></span></span><table ng-if=showTestRunDetailed class=\"table table-hover table-striped table-condensed h6\"><tbody><tr ng-repeat=\"entry in testRuns\" ng-init=\"showLogs = false\"><td width=5% ng-class=\"{none: !entry.completedAt && entry.total, error: entry.results.length > 0 && entry.results[0].failed !== 0, infor: entry.results.length > 0 && entry.results[0].failed === 0}\"><span ng-if=\"!entry.completedAt && entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-space-shuttle\" ng-attr-title=\"In Progress...\"></span> <span ng-if=\"!entry.completedAt && !entry.startedAt\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-question-circle\" ng-attr-title=\"Not run yet...\"></span> <span ng-if=\"entry.completedAt && entry.results[0].failed !== 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-thumbs-o-down error\" ng-attr-title=\"{{entry.results.length}} test outcomes\"></span> <span ng-if=\"entry.completedAt && entry.results[0].failed === 0\" ng-click=\"cmdShowTopic('testRunsDetails')\" class=\"cmd fa fa-trophy infor\" ng-attr-title=\"{{entry.results.length}} test outcomes\"></span></td><td width=5%><span title=Reload ng-click=\"publish('', 'StartUnitTest '+ entry.dll)\" class=\"cmd fa fa-rocket\"></span> <span class=popuptrigger ng-click=\"showLogs = !showLogs\">{{entry.dll}}<div class=popup><span class=popuptext ng-class=\"{show: showLogs}\"><span class=\"cmd fa fa-exclamation-circle\" ng-click=\"publish('TestArena\\\\'+entry.worker+'', 'StopUnitTest')\" title=\"STOP running tests\"></span> <span ng-repeat=\"a in entry.assets\"><a ng-href=\"{{'/tenantLogs/' + a}}\" target=_tenantlogs><span class=\"cmd fa fa-stack-overflow\" ng-attr-title={{a}}></span></a></span></span></div></span></td><td width=5% ng-if=\"entry.completedAt && entry.results[0].failed !== 0\">{{entry.results[0].failed}} / {{(entry.results[0].failed + entry.results[0].passed)}}</td><td width=5% ng-if=\"!entry.completedAt && entry.total !== 0\">{{entry.total}}</td><td width=5% ng-if=\"entry.completedAt && entry.results[0].failed === 0\">{{(entry.results[0].passed)}}</td><td width=5% ng-attr-title={{entry.results[0].duration}}>{{entry.results[0].duration}}</td><td width=80%><bar-chart class=cmd height=30 data=entry.results></bar-chart></td></tr></tbody></table></div><div class=\"pieChart gfirst\"><pie-chart class=cmd ng-click=\"cmdShowTopic('slow')\" data=testOutcomes></pie-chart><img src=img/thebfg.png ng-style=\"{'visibility': testOutcomes.length > 0 ? 'hidden' : 'visible'}\" style=\"mix-blend-mode: multiply; position: absolute; top: 112px; left:-1px\"><div class=fire style=\"border-radius: 80%;\r" +
    "\n" +
