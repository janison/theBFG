...
//todo: i really need to cleanup all this JS
//up there too ^^
    var context;
    ...
    $scope.stageSound = function(sfx) {

        return new Promise(function(resolve, reject) {           
          
            function getSFXReady(bufferList) {
                // Create two sources and play them both together.
                var passSound = context.createBufferSource();
                passSound.buffer = bufferList[0];

                passSound.connect(context.destination);
                
                resolve(function() {
                    passSound.start(context.currentTime);
                    passSound.stop(context.currentTime+1);        
                });
            }
            
            var bl = new BufferLoader(
            context,
            [
            sfx
            ],
    ...
    $scope.failSFX = function() {
        $scope.stageSound('sfx/fail.ogg').then(s => s());
    }

    $scope.passSFX = function() {
        $scope.stageSound('sfx/pass.ogg').then(s => s());        
    }
});
